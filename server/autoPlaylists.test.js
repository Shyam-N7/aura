...
// every requested id unless told to drop some; INSERTs are captured.
let editions, stored, missingTrackIds;
function installPool() {
  pool.query.mockImplementation(async (sql, params) => {
    if (sql.includes('FROM mix_editions') && sql.includes('edition_key = $3')) {
      ...
      return { rows: hit ? [{ payload: hit.payload, edition_key: hit.editionKey }] : [] };
    }
...
describe('getAutoPlaylists — edition cache', () => {
  it('serves cached editions without generating, with edition + rule metadata', async () => {
    ...
    const keys = {
      ...
      'bring-it-back': lastMondayKey(0), morning: lastMondayKey(0), night: lastMondayKey(0),
    };
...
describe('getAutoPlaylists — discovery serving', () => {
  ...
  it('serves the previous edition with refreshing:true instead of generating inline', async () => {
    ...
    editions = [{ mixKey: 'new-to-you', editionKey: '2026-06-26', payload: prev }];
  ...
  it('generates inline only for a first-ever edition, and stores it', async () => {
    getDiscoveryGate.mockResolvedValue({ ok: true, have: 80, need: 30 });
