...
describe('verifyWebhookSignature', () => {
  ...
  it('accepts a correctly signed payload', () => {
    ...
    expect(verifyWebhookSignature(body, headersWith(sig), signingSecret)).toBe(true);
  });

  it('accepts when the header lists multiple signatures (key rotation)', () => {
    const good = `v1,${sign(secretB64, id, ts, body)}`;
    expect(verifyWebhookSignature(body, headersWith(`v1,ZGVhZGJlZWY= ${good}`), signingSecret)).toBe(true);
  });

  it('rejects a tampered body', () => {
    const sig = `v1,${sign(secretB64, id, ts, body)}`;
    expect(verifyWebhookSignature(body + 'x', headersWith(sig), signingSecret)).toBe(false);
  });

  it('rejects a wrong secret', () => {
    const sig = `v1,${sign(secretB64, id, ts, body)}`;
    const other = `whsec_${Buffer.from('a-completely-different-secret-99').toString('base64')}`;
    expect(verifyWebhookSignature(body, headersWith(sig), other)).toBe(false);
  });

  it('rejects missing headers, body, or secret', () => {
    const sig = `v1,${sign(secretB64, id, ts, body)}`;
    expect(verifyWebhookSignature(body, {}, signingSecret)).toBe(false);
    expect(verifyWebhookSignature(body, headersWith(sig), '')).toBe(false);
    expect(verifyWebhookSignature(null, headersWith(sig), signingSecret)).toBe(false);
  });
});
