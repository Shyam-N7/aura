...
// the list is visible and undoable in Settings.
export async function listHidden({ signal } = {}) {
  const res = await fetchAuthed('/api/hidden', { signal });
  if (!res.ok) throw new Error(`hidden fetch failed (${res.status})`);
...
export async function unhideTrack(trackId) {
  const res = await fetchAuthed(`/api/hidden/${encodeURIComponent(trackId)}`, { method: 'DELETE' });
