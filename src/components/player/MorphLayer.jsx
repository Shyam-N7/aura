...
export function MorphLayer({ track, fromRect, toRect, kind, durationMs = 460 }) {
  // `blur` rides the same one-shot transition. Callers can pin from/to blur on
  // the rects (the mobile open holds the cover soft so the player's own banner
  // does the visible focus-in); otherwise an opening cover defaults to soft →
  // sharp as it seats ("develops into focus"), closes stay sharp.
  const [rect, setRect] = useState({ ...fromRect, radius: fromRect.radius ?? 12, blur: fromRect.blur ?? (kind === 'open' ? 8 : 0) });
  useEffect(() => {
    // Two RAFs to ensure the initial style commits before the transition starts.
    const id = requestAnimationFrame(() => requestAnimationFrame(() =>
      setRect({ ...toRect, radius: toRect.radius ?? 12, blur: toRect.blur ?? 0 })));
