...
function bump(map, p) {
  const cur = map.get(p.trackId) ?? { trackId: p.trackId, title: p.title, artist: p.artist, imageUrl: p.imageUrl ?? null, count: 0 };
  cur.count += 1;
...
export function summarizeClock(plays = [], { perPart = 3 } = {}) {
  const parts = new Map(CLOCK_PARTS.map(p => [p.key, { ...p, plays: 0, byTrack: new Map() }]));
  ...
  for (const p of plays) {
    ...
    if (d.getHours() < 5) bump(amByTrack, p);
  }
